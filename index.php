...
$langues = array(
	...
	'fr' => array(
		...
		'txtNoProjet' => 'Ben allez mon gars il faut creer des projets !!'
	)
);
...
<html lang="en">
<head>
	<title><?php echo $langues[$langue]['titreHtml']; ?></title>
...
.container {
	...
	background: rgba(216,217,237,0.92);
	position: relative;
	border-radius: 15px;
	box-shadow: 5px 5px 15px rgba(47,47,47,0.3);
	display: none;
}
#head {
	margin-bottom: 1.8em;
...
ul.projects, ul.tools {
	list-style: none;
	line-height: 24px;
}
ul.projects a, ul.tools a {
	padding-left: 22px;
...
<body>
	...
	<!--
		<ul class="utility">
			<li><a href="<?php?lang={$langues[$langue]['autreLangueLien']}?>"><?php echo $langues[$langue]['autreLangue']; ?></a></li>
		</ul>
	-->
		<h1> <?php echo strtoupper($langues[$langue]['labTitle']); ?> </h1>

		


		<ul class="utility">
			<li><a href="http://localhost:8080"> Racine </a> </li>
		</ul>
		<ul class="retour">
			<li><a href="http://localhost:8080/sites"> Retour </a> </li>
		</ul>
		...
		<ul>
			<li><a href="ConvertTXTtoHTMLIndex.php"> Améliorer la lecture de ce *** de fichier texte </a> </li>
		</ul>



		<h2> <?php echo $langues[$langue]['txtProjet']; ?></h2>
		...
		<div id="newFolder">
			<input type="text" placeholder="Nouveau dossier" autofocus="autofocus" />
		</div>
	</div>
...
<script>
	$(document).ready(function() {
		...
		$("h1").animate({right: "40em"}, { duration: 800, queue: false })
			   .delay(700).animate({letterSpacing: "0.3em"}, { duration: 400, queue: false })
			   .animate({right: "22em"}, 300)
			   .animate({letterSpacing: "0.7em"}, 400);
		...
		$('.projects li').each(function() {
		    ...
		    $(this)	.delay(i)
		    		...
		    		.animate({visibility: 'visible',opacity: 1, left: "0"}, 400);
		    i+=100;
		});


		$( ".projects li" ).hover(
			function() { $(this).animate({letterSpacing: "5px", backgroundColor: "#727FAE"}, 200); },
			function() { $(this).animate({letterSpacing: "1px", backgroundColor: "transparent"}, 200); }
		);
